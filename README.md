...
- 版本与备份：实时保存当前状态；手动保存和每 30 秒最多一次的自动检查点进入历史，可恢复最近 100 个检查点。数据库备份滚动保留 30 份每日备份和 12 份手动备份；也可导出或导入私有备份码，导入时始终创建副本。
...
4. “备份”仅用于自己的设备间迁移或留存副本。当前项目仍会实时同步；历史只保留手动保存和间隔检查点，避免频繁编辑占满服务器。备份码包含图纸状态和图片数据，可能较长，请保存在可信位置。
